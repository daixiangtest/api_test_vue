...
<script setup>
	...
	const interfaces1 = proStoreRef.interface1
	const interfaces2 = proStoreRef.interface2
	// 定义props
	...
	async function addToScent() {
		...
		console.log(prop.cases)
		let order_s = prop.cases.length;
		for (let value of checkedCase) {
			let item = { ...value };
			order_s += 1;
			alert(prop.scene)
			const response = await api.addSceneCase({ icase: item.id, scene: prop.scene.id, sort: order_s });
